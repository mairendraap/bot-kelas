const { ID_CHANNEL, TAG, ID_ROLE , ID_ROLES , CLASS_ID } = require('../config4.json');
    ...
    async execute(message, args, client) {
       ...
       if (message.channel.id != `${ID_CHANNEL}`) return message.channel.send(`Kamu bisa menggunakan command ini di <#${ID_CHANNEL}>`);
        ...
        try {
            ...
            private.send(`${message.author} _ACCEPTED_, __**${TAG}**__ Sebagai __Ketua Kelas__ di _${message.guild.name}._`);
        } catch(error) {
            console.error(error); 
        }
    },
